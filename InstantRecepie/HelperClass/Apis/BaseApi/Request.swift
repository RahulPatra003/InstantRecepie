...
class BaseAPI: NSObject {
    ...
    /**
            use to check backend api response
     - need backend response
     */
    func checkBackendResponse(withResponse response: Any?) -> (isSuccess: Bool ,message: String) {
        if (((response as? [String: Any])?["success"] as? Int) ?? 400) == 200 {
            return (isSuccess: true , message: (((response as? [String: Any])?["message"] as? String) ?? "success"))
        }else {
            return (isSuccess: false , message: (((response as? [String: Any])?["message"] as? String) ?? "Failed"))
        }
    }
